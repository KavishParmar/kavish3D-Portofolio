...
const setAnimations = (gltf: GLTF) => {
  ...
  // Only try to setup animations if the GLTF actually contains them
  if (gltf.animations && gltf.animations.length > 0) {
    ...
    if (introClip) {
      ...
      console.warn("introAnimation clip not found in GLTF");
    }
    ...
    console.warn("GLTF contains no animations; skipping animation setup.");
  }
  function startIntro() {
    if (gltf.animations && gltf.animations.length > 0) {
      const introClip = gltf.animations.find(
